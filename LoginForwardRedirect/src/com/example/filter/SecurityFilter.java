...
import com.example.request.UserRoleRequestWrapper;
...
public class SecurityFilter implements Filter {
    ...
    public void doFilter(ServletRequest req, ServletResponse resp, FilterChain chain)
        ...
        if (SecurityUtils.isSecurityPage(request)) {
            // Если пользователь еще не вошел в систему, Redirect к странице логина.
            if (loginedUser == null) {
            ...
            if (!hasPermission) {
                request.getServletContext()
                        .getRequestDispatcher("/WEB-INF/views/accessDeniedView.jsp").forward(request, response);
                return;
            }
        }
